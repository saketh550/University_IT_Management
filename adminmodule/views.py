...
from studentapp.models import StudentRegistration,Publicmessgage,FundAmount, Feedback
from facultyapp.models import FacultyRegistration
from .models import FundDetails,Eventdetails,Gallery,Resources
...
def acceptrequest(request, id):
    update = StudentRegistration.objects.get(id=id)
    update.status = "active"
    email = update.Email
    update.save()
    message = f'Hi {email},\n\nYour Authentication and key agreement based on anonymous is now active. You can login now.\n\nThis message is automatically generated, so please do not reply to this email.\n\nThank you.\n\nRegards,\nAdmin'
    subject = "PG Desertation Registration Confirmation"
...
def addresources(req):
    if req.method=="POST":
        ...
        data.save()
    return render(req,"addresources.html")


def view_feedback(request):
    all_data = Feedback.objects.all()
    print("----------------------------------------------------------------")
    return render(request, "view_all_feedback.html", {'data': all_data,})
