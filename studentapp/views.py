...
from adminmodule.models import FundDetails, Resources
from .models import StudentRegistration,Publicmessgage,PrivateChat,JobDetails,FundAmount,StudentPrivateChat, Feedback
...
def view_resources(req):
    data= Resources.objects.all()
    return render(req, 'view_resources.html', {'data':data} )

def viewlink(req,id):
    link =[i.urllink for i in Resources.objects.filter(id=id)]
    link = link[0]
...
def feedback(request):
    ...
    if request.method == "POST":
        ...
        if description: 
            feedback = Feedback(description=description, email=studentemail) 
            feedback.save()  
